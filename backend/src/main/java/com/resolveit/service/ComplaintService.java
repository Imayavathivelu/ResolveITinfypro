...
public class ComplaintService {
    ...
    private final com.resolveit.repository.EscalationRepository escalationRepository;
    ...
    public ComplaintService(ComplaintRepository complaintRepository,
            ...
            com.resolveit.repository.EscalationRepository escalationRepository) {
    ...
    public Complaint createComplaint(Complaint complaint) {
        ...
        if (recipient != null) {
            notificationService.sendNotification(recipient, "Complaint Submitted",
                    ...
                    saved.getComplaintId(), com.resolveit.model.Notification.NotificationType.STATUS_UPDATE);
        }
        return saved;
    }
    ...
    private String getNotificationRecipient(Complaint complaint) {
        if (complaint.getUser() != null) {
            return complaint.getUser().getEmail();
        } else if (Boolean.TRUE.equals(complaint.getIsAnonymous())) {
    ...
    public Complaint createComplaintWithAttachment(Complaint complaint,
        ...
        if (recipient != null) {
            notificationService.sendNotification(recipient, "Complaint Submitted",
                    ...
                    savedComplaint.getComplaintId(), com.resolveit.model.Notification.NotificationType.STATUS_UPDATE);
        }
    ...
    public Complaint assignComplaint(Long id, Long userId) {
        ...
        return complaintRepository.findById(id).map(complaint -> {
            ...
            String recipient = getNotificationRecipient(updated);
            if (recipient != null) {
                notificationService.sendNotification(recipient, "Technician Assigned", "Your complaint #"
                        + updated.getComplaintId() + " has been assigned to " + technician.getFullName(),
                        updated.getComplaintId(), com.resolveit.model.Notification.NotificationType.ASSIGNMENT);
            }
    ...
    public void checkAndEscalateComplaints() {
        ...
        // Find a fallback senior admin for escalation
        List<User> seniorAdmins = userRepository.findAll().stream()
                .filter(u -> u.getRole() == User.Role.SENIOR_ADMIN)
                .collect(java.util.stream.Collectors.toList());
        User fallbackAdmin = seniorAdmins.isEmpty() ? null : seniorAdmins.get(0);
        ...
        for (Complaint complaint : slowComplaints) {
            ...
            if (fallbackAdmin != null) {
                complaint.setAssignedTo(fallbackAdmin);
            }
            ...
            com.resolveit.model.Escalation escalation = new com.resolveit.model.Escalation();
            escalation.setComplaint(complaint);
            escalation.setEscalationReason("SLA Breach: Unresolved for > 48 hours");
            escalation.setEscalationLevel(1);
            escalation.setEscalatedTo(fallbackAdmin);
            escalationRepository.save(escalation);

            // Notify User
            String userEmail = getNotificationRecipient(complaint);
            if (userEmail != null) {
                notificationService.sendNotification(userEmail, "Complaint Escalated",
                        ...
                        complaint.getComplaintId(), com.resolveit.model.Notification.NotificationType.ESCALATION);
            }

            // Notify Original Technician (if any)
            if (complaint.getAssignedTo() != null && !seniorAdmins.contains(complaint.getAssignedTo())) {
            ...
            // Notify Senior Admin(s)
            if (!seniorAdmins.isEmpty()) {
                for (User admin : seniorAdmins) {
                    notificationService.sendNotification(admin.getEmail(),
    ...
    public Complaint resolveComplaint(Long id, String comment, String adminEmail) {
        ...
        return complaintRepository.findById(id).map(complaint -> {
            ...
            String recipient = getNotificationRecipient(updated);
            if (recipient != null) {
                notificationService.sendNotification(recipient, "Grievance Resolved", "Your grievance #"
                        + updated.getComplaintId() + " has been marked as resolved. Please review the solution.",
                        updated.getComplaintId(), com.resolveit.model.Notification.NotificationType.RESOLUTION);
            }
            return updated;
        }).orElseThrow(() -> new RuntimeException("Complaint not found"));
    }
    ...
    public Complaint reopenComplaint(Long id, String reason, String userEmail) {
        return complaintRepository.findById(id).map(complaint -> {
            ...
            String recipient = getNotificationRecipient(updated);
            if (recipient != null) {
                notificationService.sendNotification(recipient, "Complaint Reopened",
                        "Your complaint #" + updated.getComplaintId() + " has been reopened for further investigation.",
                        updated.getComplaintId(), com.resolveit.model.Notification.NotificationType.STATUS_UPDATE);
            }
            return updated;
        }).orElseThrow(() -> new RuntimeException("Complaint not found"));
    }
